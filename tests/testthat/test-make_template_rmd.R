test_that("make_template_rmd copies the template file and prints a message", {
  temp_dir <- tempdir()
  ...
  expect_output(make_template_rmd(new_template_file_name = new_template_file, template_file = template_file), "created")
...
test_that("make_template_rmd handles missing template file gracefully", {
  temp_dir <- tempdir()
  old_wd <- setwd(temp_dir)
  on.exit(setwd(old_wd))
