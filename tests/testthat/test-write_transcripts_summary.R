test_that("write_transcripts_summary writes a CSV and returns a tibble", {
  temp_dir <- tempdir()
  temp_file <- "test_transcripts_summary.csv"
  df <- tibble::tibble(
    section = c("A", "B"),
  ...
  result <- write_transcripts_summary(df, data_folder = temp_dir, transcripts_summary_file = temp_file)
...
test_that("write_transcripts_summary handles empty input", {
  temp_dir <- tempdir()
  temp_file <- "test_transcripts_summary_empty.csv"
  df <- tibble::tibble(
    section = character(),
