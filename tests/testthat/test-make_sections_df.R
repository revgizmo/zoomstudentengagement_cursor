test_that("make_sections_df counts students by section correctly", {
  ...
  # Verify exact output structure and values
  expected <- tibble::tibble(
    dept = c("ENGL", "MATH", "MATH"),
    course_num = c(201, 101, 101),
    section = c(1, 1, 2),
    n = c(1, 2, 1)
  )
  expect_equal(result %>% dplyr::arrange(dept, course_num, section), 
              expected %>% dplyr::arrange(dept, course_num, section))
})
...
test_that("make_sections_df handles invalid input", {
  expect_error(make_sections_df(NULL), "roster_df must be a tibble")
  expect_error(make_sections_df(list(a = 1)), "roster_df must be a tibble")
})
