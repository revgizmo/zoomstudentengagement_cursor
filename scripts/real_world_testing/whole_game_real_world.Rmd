  ...
  if ("participant_type" %in% names(existing_lookup)) {
    ...
    if (nrow(instructor_entries) > 0) {
      ...
      if (!current_instructor_configured) {
        ...
        cat("✅ Updated instructor configuration (preserved existing entries).\n")
      }
    } else {
      ...
      safe_update_lookup_file(updated_lookup, lookup_file)
      cat("✅ Instructor configuration added (preserved existing entries).\n")
    }
  } else {
...
cat("💡 Note: Automatic backups are created when the file is updated.\n")
}
...
if (exists("transcript_data") && exists("roster")) {
  ...
  # Read-only by default: prevent accidental overwrites of section_names_lookup.csv
    ...
    if (file.exists(lookup_file)) {
      ...
      if ("participant_type" %in% names(current_lookup)) {
        ...
        if (nrow(instructor_entries) == 0) {
          ...
          if (allow_lookup_write) {
            safe_update_lookup_file(updated_lookup, lookup_file)
            cat("✅ Instructor configuration added to lookup file (preserved existing entries).\n")
          } else {
            cat("⏩ Skipped writing (read-only). Preview only; no file changes made.\n")
          }
        } else {
