...
# Safe, read-only by default instructor configuration using package utilities
library(zoomstudentengagement)
...
cat("=== Instructor Configuration (Safe) ===\n")
...
if (nrow(existing_lookup) != nrow(updated_lookup)) {
  cat("Will add instructor row to lookup (preserving existing entries).\n")
  if (allow_lookup_write) {
    conditionally_write_lookup(updated_lookup, lookup_file, allow_write = TRUE)
    cat("✅ Instructor configuration written transactionally with backup.\n")
  } else {
    cat("⏩ Read-only mode: preview only; no file changes made.\n")
  }
} else {
...
# Privacy-aware participant identification (pure, default read-only)
if (exists("roster")) {
  cat("\n=== Privacy-Aware Participant Identification ===\n")

  allow_lookup_write <- isTRUE(params$allow_lookup_write)
  if (!allow_lookup_write) {
    cat("(Read-only mode) Will not modify data/metadata/section_names_lookup.csv.\n")
    cat("Set params: allow_lookup_write = TRUE to enable updates.\n")
  }

  # Ensure we have transcript_data
  if (!exists("transcript_data")) {
    transcript_files <- list.files(
      "data/transcripts",
      pattern = "\\.transcript\\.vtt$",
    ...
    if (length(transcript_files) > 0) {
      ...
      cat("⚠️  No transcript files found for identification\n")
    }
  }

  if (exists("transcript_data")) {
    # Read and optionally update lookup with instructor
    lookup_file <- file.path("data/metadata", "section_names_lookup.csv")
    existing_lookup <- read_lookup_safely(lookup_file)
    updated_lookup <- ensure_instructor_rows(existing_lookup, params$instructor_name)
    if (nrow(existing_lookup) != nrow(updated_lookup) && allow_lookup_write) {
      conditionally_write_lookup(updated_lookup, lookup_file, allow_write = TRUE)
      cat("✅ Lookup updated with instructor (transactional with backup).\n")
    }

    # Classify participants with FERPA-safe defaults
    set_privacy_defaults(privacy_level = "ferpa_standard")
    clean_metrics <- classify_participants(
    ...
    cat("Total utterances:", nrow(clean_metrics), "\n")
    if ("participant_type" %in% names(clean_metrics)) {
      type_counts <- table(clean_metrics$participant_type)
      cat("Participant types:\n")
      for (type in names(type_counts)) {
        cat("  ", type, ":", type_counts[type], "\n")
      }
    }
    if ("is_matched" %in% names(clean_metrics)) {
      matched_count <- sum(clean_metrics$is_matched, na.rm = TRUE)
      unmatched_count <- sum(!clean_metrics$is_matched, na.rm = TRUE)
      cat("Matched names:", matched_count, "\n")
      cat("Unmatched names:", unmatched_count, "\n")
    }
  }
} else {
  cat("⚠️  Cannot perform identification - missing roster\n")
}
```
