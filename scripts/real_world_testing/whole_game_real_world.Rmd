...
- **instructor_name**: Full name of the instructor (e.g., "Conor Healy")
- **instructor_title**: Title of the instructor (e.g., "Professor", "Dr.")
...
Rscript -e "rmarkdown::render('whole_game_real_world.Rmd', 
  params = list(instructor_name = 'Conor Healy', instructor_title = 'Professor'))"
```
  ...
  cat(sprintf("\n=== Testing %s (%s) ===\n", level, description))
    ...
    if (level == "ferpa_strict" || level == "ferpa_standard") {
      if (length(real_names) > 0) {
        cat("⚠️  WARNING: Real names found in", level, "mode\n")
      } else {
        cat("✅ Privacy level", level, "working correctly\n")
      }
    } else if (level == "mask") {
      if (length(masked_names) > 0) {
        cat("✅ Privacy level", level, "working correctly\n")
      } else {
        cat("⚠️  WARNING: No masked names found in", level, "mode\n")
      }
    } else if (level == "none") {
      if (length(real_names) > 0) {
        cat("✅ Privacy level", level, "working correctly (showing real names)\n")
      } else {
        cat("⚠️  WARNING: No real names found in", level, "mode\n")
      }
    }
    
    return(metrics)
  } else {
    cat("⚠️  No transcript files available for testing\n")
    return(NULL)
  }
}

# Test all privacy levels
cat("🔒 Testing Privacy Levels\n")
cat("========================\n")
...
cat("\n✅ Privacy testing completed. Reset to default 'mask' level.\n")
```
...
check_ferpa_compliance <- function(data, description) {
  ...
  if (any(pii_found)) {
    cat("🚨 FERPA VIOLATION: PII found in output\n")
    cat("PII indicators found:", paste(names(pii_found[pii_found]), collapse = ", "), "\n")
    return(FALSE)
  } else {
    cat("✅ FERPA compliant: No PII detected\n")
    return(TRUE)
  }
}
...
if (!is.null(mask_metrics)) {
  ...
  if (export_has_real_names) {
    cat("🚨 SECURITY ISSUE: Real names found in exported file\n")
  } else {
    cat("✅ Export security: No real names in exported file\n")
  }
...
instructor_title <- params$instructor_title

cat("=== Instructor Configuration ===\n")
cat("Instructor Name:", instructor_name, "\n")
cat("Instructor Title:", instructor_title, "\n")
...
if (file.exists(lookup_file)) {
  ...
  if ("participant_type" %in% names(existing_lookup)) {
    ...
    if (nrow(instructor_entries) > 0) {
      cat("✅ Instructors already configured:\n")
      ...
      if (!current_instructor_configured) {
        cat("⚠️  Current instructor not found in existing configuration.\n")
        cat("Adding current instructor to configuration...\n")
        
        # Add current instructor to existing configuration
        new_instructor_entries <- create_instructor_entries(instructor_name, instructor_title)
        updated_lookup <- rbind(existing_lookup, new_instructor_entries)
        write.csv(updated_lookup, lookup_file, row.names = FALSE)
        cat("✅ Updated instructor configuration.\n")
      }
    } else {
      cat("⚠️  No instructors configured in lookup file\n")
      cat("Adding current instructor to configuration...\n")
      
      # Create instructor entries and save
      instructor_lookup <- create_instructor_entries(instructor_name, instructor_title)
      write.csv(instructor_lookup, lookup_file, row.names = FALSE)
      cat("✅ Instructor configuration created.\n")
    }
  } else {
    cat("⚠️  Lookup file missing participant_type column.\n")
    cat("Creating new lookup file with instructor configuration...\n")
    
    # Create new lookup file with instructor entries
    instructor_lookup <- create_instructor_entries(instructor_name, instructor_title)
    write.csv(instructor_lookup, lookup_file, row.names = FALSE)
    cat("✅ Instructor configuration created.\n")
  }
  ...
  instructor_lookup <- create_instructor_entries(instructor_name, instructor_title)
  write.csv(instructor_lookup, lookup_file, row.names = FALSE)
  cat("✅ Instructor configuration created:", lookup_file, "\n")
...
create_instructor_entries <- function(name, title) {
  # Create common variations of the instructor name
  variations <- c(
    name,  # Full name
    paste(title, gsub("^.*\\s+", "", name)),  # Title + Last name
    paste("Dr.", gsub("^.*\\s+", "", name)),  # Dr. + Last name
    paste("Professor", gsub("^.*\\s+", "", name))  # Professor + Last name
  )
  ...
  data.frame(
    transcript_name = variations,
    preferred_name = rep(name, length(variations)),
    formal_name = rep(name, length(variations)),
    participant_type = rep("instructor", length(variations)),
    student_id = rep("INSTRUCTOR", length(variations)),
    notes = c("Primary instructor name", rep("Title variation", length(variations) - 1)),
    stringsAsFactors = FALSE
  )
}
...
cat("\nCurrent instructor variations configured:\n")
if (exists("instructor_lookup")) {
  print(instructor_lookup[, c("transcript_name", "preferred_name", "participant_type")])
...
if (file.exists(roster_path)) {
  tryCatch({
    ...
    cat("⚠️  Error loading roster:", e$message, "\n")
    cat("Please check that your roster.csv file is properly formatted\n")
  })
} else {
  cat("⚠️  Roster file not found at:", roster_path, "\n")
...
if (file.exists(lookup_file)) {
  ...
  if ("participant_type" %in% names(lookup_data)) {
    ...
    if (nrow(instructor_entries) > 0) {
      cat("✅ Instructors configured in lookup file:\n")
      print(instructor_entries[, c("transcript_name", "preferred_name", "participant_type")])
      ...
      if (exists("transcript_data")) {
        ...
        if (length(matching_instructors) > 0) {
          cat("✅ Found configured instructors in transcript:", paste(matching_instructors, collapse = ", "), "\n")
        } else {
          cat("⚠️  Configured instructors not found in transcript.\n")
  ...
  cat("⚠️  No section_names_lookup.csv found.\n")
...
if (exists("transcript_data") && exists("roster")) {
  ...
  if (length(transcript_files) > 0) {
    ...
    tryCatch({
      ...
      cat("✅ Name matching workflow completed successfully!\n")
      ...
      if (!is.null(result) && nrow(result) > 0) {
        ...
        cat("⚠️  No results returned from name matching workflow\n")
      }
      
    }, error = function(e) {
      cat("❌ Error in name matching workflow:", e$message, "\n")
    ...
    cat("⚠️  No transcript files found for name matching\n")
  }
} else {
  cat("⚠️  Cannot perform name matching - missing transcript data or roster\n")
}
```
...
if (length(mapping_files) > 0) {
  ...
  tryCatch({
    ...
    cat("❌ Error loading custom mappings:", e$message, "\n")
  })
} else {
...
if (exists("student_summary") && nrow(student_summary) > 0) {
  ...
  if ("utterance_count" %in% names(student_summary)) {
    ...
    cat("⚠️  Cannot identify low participation - no suitable metrics available\n")
    low_participation <- student_summary
  }
  ...
  if ("total_duration" %in% names(low_participation)) {
    print(low_participation[, c(name_col, "utterance_count", "total_duration")])
  } else {
    print(low_participation[, c(name_col, "utterance_count")])
  }
...
scan_for_real_names <- function(data, description) {
  ...
  if (length(real_names) > 0) {
    cat("🚨 PRIVACY ISSUE: Real names found in final output\n")
    cat("Names found:", paste(unique(real_names), collapse = ", "), "\n")
    cat("This violates FERPA compliance requirements.\n")
    return(FALSE)
  } else {
    cat("✅ Privacy maintained: No real names in final output\n")
    return(TRUE)
  }
}
...
cat("\n✅ Final privacy validation completed.\n")
```
